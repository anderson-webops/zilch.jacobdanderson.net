...
- Production activation is artifact-only. Never rebuild source in a production release tree or depend on the removed `.zilch-runtime.sha256` format. Verify the successful tagged workflow, unchanged archive, installed contract, distinct-UID readability, disposable-port startup, and graceful shutdown before promotion.
- Preserve the exact version-aware rollback exception: only v1.4.1 at `fc43e474c0c402fdea39828e02a59cab9aa60661` may use legacy `/api/health` and the historical Nginx server block. All artifact-era releases require full liveness and readiness gates.
