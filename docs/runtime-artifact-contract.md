...
A passing artifact test does not authorize activation. Use only the existing reviewed promotion mechanism in [`deploy/README.md`](../deploy/README.md), retain the exact prior release and its root-owned versioned helper, and separately verify live identity, health, readiness, TLS, and both public address families. Rollback selects the retained release's protected contract and Nginx template while continuing to use the newest trusted verifier. A server-origin probe using local address resolution or a NAT hairpin is an origin check, not independent external-network acceptance. `builtAt` records artifact preparation time, not production activation.
