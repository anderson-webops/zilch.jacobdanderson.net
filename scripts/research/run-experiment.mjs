...
const study = parameters[0] === '--study' ? parameters.splice(0, 2)[1] : 'hot-dice-2026-09'
assert(study && /^[a-z0-9-]+$/.test(study), 'Study must be a simple lowercase directory name.')
const [simulatorRoot, executable, runId, ...args] = parameters
...
const output = resolve(root, 'docs/research', study, 'results', `${runId}.json`)
...
const primary = result.mode === 'state'
  ? result.roll_minus_bank_match_points_paired
  : result.mode === 'selection'
    ? result.right_minus_left_match_points_paired
    : result.a_match_point_rate_paired
